...
var lookbookCreate = true;
var imageLink = "";

function safeLookbook() {
  var lookbook = lookbookStruct;
  if(lookbookCreate)
  ...
  setTimeout(function(){ window.location.href = "lookbook.html";}, 3000);
  if(lookbookCreate)
...
function getIdOfUser() {
    var currentUser = window.localStorage.getItem('user').split('"')[3];
    for(var i = 0; i < getNewId(); i++) {
      let lookbookies = window.localStorage.getItem('lookbook' + i).split(",");
      var user = (lookbookies[1] + lookbookies[2]).split('\"')[6].split('\\')[0];
      if(user == currentUser) {
        return i;
      }
    }
...
function doesLookbooksExists(){
  ...
  for(var i = 0; i < getNewId(); i++) {
    ...
    if(user == currentUser) {
      ...
      var id = lookbookies[0].split(":")[1].split('"')[0];
      var headline = lookbookies[3].split(":")[1].split('"')[1];
      var price = lookbookies[4].split(":")[1].split('"')[1];
      var desc = lookbookies[5].split(":")[1].split('"')[1];
      var user = (lookbookies[1] + lookbookies[2]).split('\"')[6].split('\\')[0];
